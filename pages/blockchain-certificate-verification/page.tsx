...
const Finished = () => {
  ...
  const [brideNID, setBrideNID] = React.useState<string>('');
  const [groomNID, setGroomNID] = React.useState<string>('');
  ...
  const fetchAndFilterNfts = async () => {
    ...
    try {
      ...
      if (result && result.items) {
        ...
        // Filter NFTs based on input NIDs
        const filtered = fetchedNfts.filter((nft: Nft) => {
          ...
          return (!brideNID || nftBrideNID === brideNID) && (!groomNID || nftGroomNID === groomNID);
        });
      ...
      <div className='rounded-lg min-h-content bg-[#2a302f] p-4 sm:col-span-6 lg:col-start-2 lg:col-end-6'>
        <div className='flex justify-between items-center'>
          <h2 className='text-lg sm:text-2xl font-semibold'>
            Blockchain certificate Verification          </h2>
          <button
        ...
        <div className='mt-8'>
          ...
          <input
            ...
            className='w-full p-4 rounded-lg border border-neutral-800 bg-neutral-950 placeholder:text-neutral-700 text-white mt-4'
          />
        </div>
